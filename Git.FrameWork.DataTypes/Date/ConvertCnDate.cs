...
namespace Git.Framework.DataTypes.Date
{
    public class ConvertCnDate
    {
        ...
        private static long[] calElement(int y, int m, int d)
        {
            ...
            DateTime time2 = new DateTime(y, m, d);
            ...
            {
                if (((num3 > 0) && (num == (num3 + 1))) && (numArray[6] == 0L))
                {
                    num--;
                    numArray[6] = 1L;
        ...
        public static ChinaDate getChinaDate(DateTime dt)
        {
            ChinaDate date = new ChinaDate();
            int year = dt.Year;
            int month = dt.Month;
            int day = dt.Day;
            ...
            date.cnStrMonth = nStr1[(int) numArray[1]];
            ...
            foreach (string str4 in sFtv)
            {
                if (str4.Substring(0, 4).Equals(str))
                {
                    date.cnFtvs = str4.Substring(4, str4.Length - 4);
                    break;
                }
            }
            foreach (string str6 in lFtv)
            {
                if (str6.Substring(0, 4).Equals(str2))
                {
                    date.cnFtvl = str6.Substring(4, str6.Length - 4);
                    break;
                }
            }
            dt = dt.AddDays(1.0);
        ...
        private static int leapDays(int y)
        {
            if (leapMonth(y) > 0)
            {
                if ((lunarInfo[y - 0x76c] & 0x10000L) > 0L)
                {
                    return 30;
                }
                return 0x1d;
        ...
        private long[] Lunar(int y, int m)
        {
            ...
            DateTime time2 = new DateTime(y + 0x76c, m + 1, 1);
            TimeSpan span = (TimeSpan) (time2 - time);
            long totalDays = (long) span.TotalDays;
            if (y < 0x7d0)
            {
        ...
        {
            if ((lunarInfo[y - 0x76c] & (((int) 0x10000) >> m)) == 0L)
            {
                return 0x1d;
            }
            return 30;
